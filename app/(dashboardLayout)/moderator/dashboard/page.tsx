...
export default function ModeratorDashboardPage() {
  const modStats = [
    { label: "Ideas Reviewed", value: "1,204", icon: CheckCircle, change: "+8.4%", color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Active Members", value: "2,468", icon: Users, change: "+4.2%", color: "text-blue-600", bg: "bg-blue-50" },
    { label: "Reported Comments", value: "24", icon: MessageSquareWarning, change: "-12%", color: "text-rose-600", bg: "bg-rose-50" },
    { label: "Pending Approvals", value: "15", icon: AlertTriangle, change: "+2%", color: "text-amber-600", bg: "bg-amber-50" },
  ];

  const recentActivities = [
    { user: "Alice Green", action: "Reported Comment", item: "Post #1042", status: "Pending", date: "2 mins ago" },
    { user: "Bob Smith", action: "Idea Submitted", item: "Solar Panel Design", status: "Reviewed", date: "15 mins ago" },
    { user: "EcoVentures", action: "Requested Category", item: "Wind Energy", status: "Approved", date: "1 hour ago" },
    { user: "Sarah Sun", action: "Appealed Warning", item: "Warning #023", status: "Resolved", date: "3 hours ago" },
  ];
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col justify-between gap-6 border-b pb-8 md:flex-row md:items-center">
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight text-neutral-900 font-sans">Moderator Overview</h1>
          <p className="mt-2 text-lg text-neutral-500 font-medium tracking-tight">Community management and moderation activities.</p>
        </div>
        ...
        <Card className="lg:col-span-2 rounded-[2.5rem] border-none bg-white shadow-sm overflow-hidden relative group">
          <CardHeader className="p-10 pb-4 flex flex-row items-center justify-between">
            <div>
              ...
              <CardDescription className="text-sm font-medium mt-1">Real-time moderation monitoring</CardDescription>
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent border-neutral-100">
                  ...
                  <TableHead className="text-xs font-black uppercase tracking-widest text-neutral-400">Action</TableHead>
                  <TableHead className="text-xs font-black uppercase tracking-widest text-neutral-400">Target Item</TableHead>
              ...
              <TableBody>
                {recentActivities.map((item, i) => (
                  <TableRow key={i} className="group/row hover:bg-neutral-50/50 border-neutral-50 transition-colors">
                    ...
                    <TableCell className="py-6 font-medium text-neutral-500">{item.action}</TableCell>
                    <TableCell className="py-6 font-black text-neutral-700">{item.item}</TableCell>
                    <TableCell className="py-6">
                      <Badge className={`rounded-xl px-3 py-1 text-[10px] font-black uppercase tracking-tighter shadow-none ${item.status === 'Resolved' || item.status === 'Approved' || item.status === 'Reviewed' ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' : 'bg-amber-100 text-amber-700 hover:bg-amber-200'}`}>
                        {item.status}
                      </Badge>
                    </TableCell>
        ...
        <div className="flex flex-col gap-8">
          <Card className="rounded-[2.5rem] bg-neutral-950 p-10 text-white relative overflow-hidden group border-none shadow-xl">
            ...
            <CardHeader className="p-0 mb-6">
              ...
              <CardTitle className="text-2xl font-black">Weekly Checks</CardTitle>
              <CardDescription className="text-neutral-400 text-sm font-medium leading-relaxed">
                You have resolved 124 moderation queries this week.
              </CardDescription>
            </CardHeader>
            <CardContent className="p-0 pt-4">
              <div className="flex items-center gap-6">
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Resolution Rate</span>
                </div>
              </div>
            </CardContent>
          </Card>
